...
class golfHTTPServer(BaseHTTPRequestHandler):
	...
	def do_POST(self):
		...
		# Send message back to client
		tmp = urlparse(self.path).query
		tmp = tmp.replace(";","")
		query_components = parse_qs(tmp)
...
def writeCom(param):
	...
	if param:
		...
		docs = col.find({})
		res = dumps(param)
		return dumps({"valid": True})	# Ok

	else:
		return dumps( {"valid": False} )	# No param
...
def readCommandQuery(query):
	...
	def addInfo(j):
		for ind in range(10):
			i = str(ind)
			tmpVal = param[('J' + j + i)] if ('J' + j + i) in param else ""
			if len(tmpVal) > 0:
				for x in range(len(tmpVal)):
					#print(tmpVal[x])
					InfoArr.append(tmpVal[x])
					mailInfo(M1, M3, int(j), tmpVal[x])	
...
def writeToSheet(infoG3):
  ...
  #try { 
	...
	if len(idInfo) > 0:  # Update
		idInfo = getID(idInfo)
		doc = coll.update({ '_id': idInfo}, { '$set':{"Time": InfoArr[0] , "Date": InfoArr[1], "IP": InfoArr[2] , "Name": InfoArr[3], "Address": InfoArr[4], "email": InfoArr[5], "ID": idInfo, "Choix": choix, "info": info}})
		Mdata = formatMailData(HOSTclient, InfoArr[1], InfoArr[3], InfoArr[5], str(idInfo), infoG3['m1'], infoG3['m3'], m1Info[0], m3Info[0])
		confirmMail('Modification de la commande de ' + InfoArr[3], InfoArr, Mdata);
		print('writeToSheet1')
	else:		# Append new
		doc = coll.insert({"Time": InfoArr[0] , "Date": InfoArr[1], "IP": InfoArr[2] , "Name": InfoArr[3], "Address": InfoArr[4], "email": InfoArr[5], "ID": idInfo, "Choix": choix, "info": info}, {"new":True})
		...
		confirmMail('Commande de ' + InfoArr[3], InfoArr, Mdata);
		print('writeToSheet2')
	#pdb.set_trace()
	#return dumps({"ok": 1, "id": o_id})
	if  isinstance(doc,(ObjectId)) or doc['ok'] == 1.0:
		return dumps({"ok": 1})
	else:
...
def confirmMail(subject, InfoArr, Mdata):
	""" // Confirm email """
	send_email("Traiteur Simpas <" + MAIL_USER + ">", InfoArr[3] + " <" + InfoArr[5] + ">", subject, "", Mdata['Mbody'])
...
def send_email(fromuser, recipient, subject, text, html):
	...
	mail.sendmail(fromuser, toaddrs, msg.as_string())
	mail.quit()
